...
std::vector<std::string> TmixShuffle::ShuffleTraces (double& scale, double& simtime, int binsecs, std::vector<std::string> tmixBaseCVName, bool findstats, bool findtarget, double prefillT, double bps, int ccTmixSrcs, int maxrtt, double targetload, direction targetdirection, double longflowthresh, int mss, int pktoh, double balancetol, double loadtol)
{
  m_pktoh = pktoh;
  m_mssI = mss;
  m_mssA = mss;
                  ...
                  if (line[0] == 'S' || line[0] == 'C')
                    {
                      ...
                      if (connout.size () > 0)
                        {
                          ...
                          //std::cout<<"bacche ki jaan loge "<< scaled_conntime << " "<< m_prefillus <<"\n";
                      ...
                      m_CVtype = line[0];
                      char l1[200], str_l2[200], l3[200], l4[200], l5[200];
                      float l2;
                      sscanf (line,"%s%f%s%s%s", l1, &l2, l3, l4, l5);
                      sprintf (str_l2,"%f",(l2 + (int)offset_time));
...
{
  TmixShuffle ts;
  int core_delay = 2,BottleneckCapacity;
  std::vector<int> buffer_delay;
  buffer_delay.push_back (102);
  buffer_delay.push_back (102);
  double warmup,maxtrace,testtime;

  double scale, simtime, prefillT, bps, targetload, longflowthresh,balancetol, loadtol;
  int binsecs, ccTmixSrcs, maxrtt, mss, pktoh;
  std::vector<std::string> tmixBaseCVName;
  bool findstats, findtarget;
  TmixShuffle::direction targetdirection = TmixShuffle::direction (2);
  scale = 5.276;
  testtime = 479;       //in seconds
  ...
  maxrtt = ( 204 +  (100.0 + core_delay) * 2.0) / 1000.0;          //convert 204 to array elements
  targetload = 0.60;       //60%
  warmup = 84.0;
  prefillT = 36.72;       //in seconds
  //prefill_si = 19.445;
